package com.mobility.enp.data.model.new_toll_history.entity

import androidx.room.Entity

@Entity(
    tableName = "new_toll_history_items",
    primaryKeys = ["id"]
)
data class TollHistoryItemEntity(
    val id: Int,
    val tagsSerialNumber: String,
    val tollPlaza: String,
    val isPaid: Boolean,
    val checkInDate: String?,       // 2026-03-18 08:29:48
    val checkOutDate: String?,      // 2026-03-18 08:53:46
    val amountWithOutDiscount: String?,
    val currency: String?,
    val billFinal: String?,         // "737995/2026-web" (iz bill objekta)
    val hasComplaint: Boolean,      // complaint != null
    val complaintId: Int?,          // complaint?.id
    val countryCode: String         // country filter koji sam koristio pri fetchu
    )
